...
/-! ## Fresh ordered compiler adapter

The definitions in this block are the strict entry point for future Hoare-triple
preservation: the schedule is the literal `gateOrdering` list, each scheduled
qubit is paired with the Pauli stored in the source stabilizer, and helper
qubits are generated by `compileFreshGadget` rather than supplied by the user.

The older `toCircuitStabilizerX/Z` functions below are kept as compatibility
wrappers for the existing standard-scheme lemmas; they intentionally use the
old `Circuit (n + 1)` shared-ancilla layout. -/

def xzOfPauli : Pauli -> Option XZPauli
  | .X => some .X
  | .Z => some .Z
  | _ => none

def orderedSlotsOf {n : Nat} (stab : ErrorVec n) :
    List (Fin n) -> Option (List (ScheduledPauli n))
  | [] => some []
  | q :: qs =>
      match xzOfPauli (stab q), orderedSlotsOf stab qs with
      | some kind, some rest => some (⟨kind, q⟩ :: rest)
      | _, _ => none

def orderedScheduleOf (spec : CodeSpec) (s : Fin spec.numStab) :
    Option (RuleSchedule spec.n) :=
  match orderedSlotsOf (spec.stabilizers s) (spec.gateOrdering s) with
  | some slots => some ⟨slots⟩
  | none => none

/-- A fresh compiled circuit packages its helper budget in the first component.
The second component then lives on `spec.n + k` qubits. -/
abbrev FreshCircuit (n : Nat) := Sigma (fun k => FCircuit (n + k))

def compileFreshStabilizer? (scheme : Scheme) (spec : CodeSpec)
    (s : Fin spec.numStab) : Option (FreshCircuit spec.n) :=
  match orderedScheduleOf spec s with
  | some sigma => some ⟨helperCount scheme sigma, compileFreshGadget scheme sigma⟩
  | none => none
